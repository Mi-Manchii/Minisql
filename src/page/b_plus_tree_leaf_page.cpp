...
int LeafPage::Insert(GenericKey *key, const RowId &value, const KeyManager &KM) {
  int insert_pos = KeyIndex(key, KM);
  int size = GetSize();
  memmove(PairPtrAt(insert_pos + 1), PairPtrAt(insert_pos), (size - insert_pos) * pair_size);
...
void LeafPage::MoveHalfTo(LeafPage *recipient) {
  ...
  recipient->CopyNFrom(PairPtrAt(move_size), size - move_size);
  SetSize(move_size);
}
...
int LeafPage::RemoveAndDeleteRecord(const GenericKey *key, const KeyManager &KM) {
  int remove_index = KeyIndex(key, KM);
  int size = GetSize();
  if (remove_index >= size || comparator(key, KeyAt(remove_index)) != 0)
    return size;
  memmove(PairPtrAt(remove_index), PairPtrAt(remove_index + 1), (size - remove_index - 1) * pair_size);
  IncreaseSize(-1);
  return size - 1;
}
...
void LeafPage::MoveLastToFrontOf(LeafPage *recipient) {
  GenericKey *last_key = KeyAt(GetSize() - 1);
  RowId last_value = ValueAt(GetSize() - 1);
  IncreaseSize(-1);
...
void LeafPage::CopyFirstFrom(GenericKey *key, const RowId value) {
  int size = GetSize();
  memmove(PairPtrAt(1), PairPtrAt(0), size * pair_size);
  SetKeyAt(0, key);
  SetValueAt(0, value);
  IncreaseSize(1);
}